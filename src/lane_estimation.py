...
class estimation:
    ...
    def __call__(self, image_binary):
        return self.get_mid_poly(image_binary, False, False)

    def get_mid_poly(self, img, is_ver, is_show=False):
        f_array = img[125, :]
        self.left_point = self.find_point(f_array, self.mid, "left")
        if self.left_point == None:
            self.left_point = self.prev_left_point
        self.right_point = self.find_point(f_array, self.mid, "right")
    ...
    def find_point(self, f_array, start, mode):
        stack = 0
        if mode == "right":
            while start < 500:
                if f_array[start] == 0:
